...
class Mai_Guides_Display {
	...
	function get_guides() {
		...
		$html .= '<div class="mai-guides-container">';
			$html .= '<ul class="mai-guides">';
				$html .= $this->get_guide( $guide_id, false, true );
				$i = 1;
				foreach ( $post_ids as $post_id ) {
					$html .= $this->get_guide( $post_id, $i );
					$i++;
				}
			$html .= '</ul>';
		$html .= '</div>';
	 ...
	 * @param   bool    $is_main  Whether the guide is the main guide or a post.
	 *
	 * @return  string
	 */
	function get_guide( $post_id, $count = '', $is_main = false ) {
